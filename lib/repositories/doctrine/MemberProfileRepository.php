...
class MemberProfileRepository extends opAccessControlDoctrineRepository
{
  ...
  {
    $profileId = $this->_em->getRepository('Profile')->getIdByName($profileName);
    return $this->findOneBy(array('member_id' => $memberId, 'profile_id' => $profileId));
  ...
  public function searchMemberIds($profile = array(), $ids = null, $isCheckPublicFlag = true)
  {
    ...
    {
      $item = $this->_em->getRepository('Profile')->retrieveByName($key);
      $_result = array();
      $column = 'value';
